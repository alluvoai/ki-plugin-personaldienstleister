...
All notable changes to the **alluvo** plugin (formerly `alluvo-operator`) are
documented here. Format follows [Keep a Changelog](https://keepachangelog.com/).
The version is the **alluvo MCP server version** — it is written by the
generator, never hand-edited here.

## [Unreleased]

### Changed — 2026-09-09 the plugin is now generated: renamed to `alluvo`, every skill a thin stub (alluvo#4943)

The whole marketplace tree is generated from the api repo by
`bin/adev artisan mcp:build-operator-plugin` and replaces the hand-written
`plugins/alluvo-operator/`. Concretely:

- **The plugin is `alluvo`, the marketplace is `alluvoai`.** Skills surface as
  `alluvo:bench-check` (was `alluvo-operator:bench-check`). Existing installs of
  `alluvo-operator@alluvo-marketplace` must re-add the marketplace and install
  `alluvo@alluvoai` — this is a breaking change for installed operators.
- **Every one of the 26 skills is a thin stub.** A stub carries only its
  frontmatter (the trigger phrases, which is all the client reads up front) and the
  instruction to call `get-workflow-guidance` with its `workflow` slug and follow
  exactly what comes back — plus: relay `MODULE_LOCKED` to the operator and stop.
  The workflow prose is no longer shipped, so activating a skill no longer injects
  the full guidance into the conversation.
- **Guidance is served live by the alluvo assistant** from
  `resources/mcp/workflows/<slug>/SKILL.md` and follows the organization's plan — a
  workflow outside it answers with the plan boundary. Workflows therefore update on
  an alluvo deploy, with nothing to reinstall.
- **`using-alluvo-operator` routes and no longer restates conventions.** It calls
  `get-workflow-guidance` with no arguments for the live catalogue (including what a
  higher Tarif would unlock), then again with the chosen `workflow`; the
  cross-cutting rules — model-type resolution, the `confirmed` preview gate,
  addresses, tagging, archiving, permissions — are served as the
  `using-alluvo-operator` workflow itself, sectioned.
- **Codex is supported from the same tree.** Added
  `plugins/alluvo/.codex-plugin/plugin.json` and `.agents/plugins/marketplace.json`
  alongside the Claude manifests, plus a bundled `plugins/alluvo/.mcp.json` (OAuth,
  `https://api.alluvo.ai/mcp`) that both halves point at, so the two products can
  never drift to a different version or skill set.
- **Skill prose is now edited in the api repo** (`resources/mcp/workflows/`), not
  here. This repository is generated output: regenerate rather than hand-edit.
  `.gitattributes` now union-merges `plugins/alluvo/CHANGELOG.md` at its new path.
